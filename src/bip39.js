...
const WORD_MAP = new Map(WORDLIST.map((word, idx) => [word, idx]));
...
export function generateMnemonic(entropyBits = 256) {
  ...
  for (let j = 0; j < checksumBits; j++) {
    bits.push((hash[0] >> (7 - j)) & 1);
  }
...
export function validateMnemonic(rawMnemonic) {
  ...
  const checksumBitsCount = n / 3;
  const entropyBitsCount = n * 11 - checksumBitsCount;

  const entropy = new Uint8Array(entropyBitsCount / 8);
  for (let i = 0; i < entropyBitsCount; i++) {
    if (bits[i]) {
  ...
  for (let i = 0; i < checksumBitsCount; i++) {
    const expectedBit = (hash[0] >> (7 - i)) & 1;
    if (bits[entropyBitsCount + i] !== expectedBit) {
      return false;
    }
  }
...
export function mnemonicToSeedSync(rawMnemonic, passphrase = '') {
  const mnemonic = normalizeMnemonic(rawMnemonic);
  if (!validateMnemonic(mnemonic)) {
    throw new Error('Invalid BIP-39 mnemonic phrase');
  }

  const passwordBytes = stringToBytes(mnemonic);
  const saltBytes = stringToBytes(`mnemonic${passphrase}`);
